...
int ChangeMode(int module,int mode)
{
    ...
    //Sync RTM SRTM : resynchronises to the active edge, captures nsamples : Which is required for NPDG!	   
}
...
int ChangeConfig(void)
{
	...
	for(int i=0;i<5;i++)
	{
	    ...
	    if(success==5)
	    {
		cout<<"DAQ "<<daq[i]<<" successfully set to NPDG default setting"<<endl;
	    }
	    else
	    {
