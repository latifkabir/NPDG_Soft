...
void RunSingle(int module=MODULE,int runlength=RUN_LENGTH,int runNumber=RUN_NUMBER)
{
    ...
    signal(SIGTERM, HandleTermination);   //Handle signal generated by kill command to terminate the program.
    signal(SIGHUP, HandleTermination);   //Handle signal generated when controlling termial is terminated by clicking x button.
...
void RunAll (int runlength=RUN_LENGTH,int runNumber=RUN_NUMBER)
{
	...
	if(ready)
	{
		...
		{
		    cout<<"\t\tPlease wait while dirty DAQ data is being processed ... ..."<<endl;
		    ProcessData(newrun,DAQ30,false);
		}
		UpdateRun();
		cout<<"\n\t\tPhew!!! Done with run number : "<<newrun<<endl<<endl;
