...
void HandleTerminationGui(int signum)  //Handle forced termination
{
...
int RunSingleGui(int module=MODULE,int runlength=RUN_LENGTH,int runNumber=RUN_NUMBER)
{
    ...
    signal(SIGTERM, HandleTerminationGui);   //Handle signal generated by kill command to terminate the program.
    signal(SIGHUP, HandleTerminationGui);   //Handle signal generated when controlling termial is terminated by clicking x button.
...
int RunAllGui (int runlength=RUN_LENGTH,int runNumber=RUN_NUMBER)
{
	...
	if(ready)
	{
	    ...
	    if(!daq21.CheckStatus() && !daq30.CheckStatus())
	    {
		RunList(true);
		time_t time_now;
		time(&time_now);
		...
		if(daq21.GetFileSize()<tol*daq21.filesize  || daq30.GetFileSize()<tol*daq30.filesize)
		{
		      mvprintw(base_y2+8, base_x2+21, "PROBLEM WITH MODULES ,INCORRECT FILE SIZE");
		      refresh();
		      sleep(1);
		...
		mvprintw(base_y2+19, base_x2, "Last Run Status:");
	          ...
	          if(Rename(newrun,DAQ21,true)==1 && Rename(newrun,DAQ30,true)==1)
		  {
		      mvprintw(base_y2+20, base_x2, "Data Files for run number %d  have been renamed successfully.",newrun);
		  }
		  else
