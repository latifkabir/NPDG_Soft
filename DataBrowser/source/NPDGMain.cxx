...
///////////////////////////////////////////////////////////////////////
//     This class provides the Graphical User Interface for NPDG        //
...
// Readme File and in the NPDGamma on-line analysis technical notes         //
//                                                                          //
...
NPDGMain::NPDGMain(const TGWindow *p, ClineArgs clargs, UInt_t w, UInt_t h) :
		TGMainFrame(p, w, h) {
	//set_new_handler(0);
	...
	MakeUtilityLayout();
	MakeMainTab();
	SetWindowName("NPDG Data Analysis");
	MapSubwindows();
...
void NPDGMain::MakeUtilityLayout() {
	...
	fNumber = new TGNumberEntry(fF1, 0, 9, 999999, TGNumberFormat::kNESInteger,
			...
			999999);
	fNumber->Connect("ValueSet(Long_t)", "NPDGMain", this, "DoSetLabel()");
	...
	txt1->Connect("Clicked()", "NPDGMain", this, "DoSetLabel()");
	...
	txt2->Connect("Clicked()", "NPDGMain", this, "OpenRun()");
	...
	txt3->Connect("Clicked()", "NPDGMain", this, "OpenNextRun()");
...
void NPDGMain::MakeMainTab() {
	...
	dMainCanvas->GetCanvas()->Connect("Picked(TPad*, TObject*, Int_t)",
			"NPDGMain", this, "PadIsPicked(TPad*, TObject*, Int_t)");

}

void NPDGMain::MakeDetectorTab1() {
	if (TabActive("RING1"))
		return;
	...
	TGCompositeFrame *tf = dTab->AddTab("RING1");
	...
	dMenuBar->AddPopup("&RING1", dDetMenu1, dMenuBarItemLayout);
...
void NPDGMain::MakeDetectorTab2() {
	if (TabActive("RING2"))
		return;
	...
	TGCompositeFrame *tf = dTab->AddTab("RING2");
	...
	dMenuBar->AddPopup("&RING2", dDetMenu2, dMenuBarItemLayout);
	dMenuBar->MapSubwindows();
	dMenuBar->Layout();
...
void NPDGMain::MakeDetectorTab3() {
	if (TabActive("RING3"))
		return;
	...
	TGCompositeFrame *tf = dTab->AddTab("RING3");
	...
	dMenuBar->AddPopup("&RING3", dDetMenu3, dMenuBarItemLayout);
	dMenuBar->MapSubwindows();
	dMenuBar->Layout();
...
void NPDGMain::MakeDetectorTab4() {
	if (TabActive("RING4"))
		return;
	...
	TGCompositeFrame *tf = dTab->AddTab("RING4");
	...
	dMenuBar->AddPopup("&RING4", dDetMenu4, dMenuBarItemLayout);
	dMenuBar->MapSubwindows();
	dMenuBar->Layout();
...
void NPDGMain::MakeLogTab() {
	...
	dLogEdit->Connect("Opened()", "NPDGMain", this, "LogOpened()");
	dLogEdit->Connect("Closed()", "NPDGMain", this, "LogClosed()");
	dLogEdit->Connect("Saved()", "NPDGMain", this, "LogSaved()");
	dLogEdit->Connect("SavedAs()", "NPDGMain", this, "LogSavedAs()");
...
void NPDGMain::MakeSpinFlipperTab() {
...
void NPDGMain::RemoveDetectorTab1() {
	if (!TabActive("RING1"))
		return;
	int tab = GetTabIndex("RING1");
	if (tab < 0)
		return;
...
void NPDGMain::RemoveDetectorTab2() {
	if (!TabActive("RING2"))
		return;
	int tab = GetTabIndex("RING2");
	if (tab < 0)
		return;
...
void NPDGMain::RemoveDetectorTab3() {
	if (!TabActive("RING3"))
		return;
	int tab = GetTabIndex("RING3");
	if (tab < 0)
		return;
...
void NPDGMain::RemoveDetectorTab4() {
	if (!TabActive("RING4"))
		return;
	int tab = GetTabIndex("RING4");
	if (tab < 0)
		return;
...
void NPDGMain::RemoveMonitorTab() {
	if (!TabActive("Monitors"))
		return;
...
void NPDGMain::RemoveSpinFlipperTab() {
	if (!TabActive("Spin Flipper"))
		return;
...
TCanvas *NPDGMain::SplitCanvas(TRootEmbeddedCanvas *cnv, int r, int c,
		const char* ttip) {
...
void NPDGMain::LogClosed() {
	SetLogFileOpen (kFalse);
	SetLogFileName("");
}

void NPDGMain::LogSaved() {
	SetLogFileOpen (kTrue);
	Bool_t untitled =
			!strlen(dLogEdit->GetText()->GetFileName()) ? kTrue : kFalse;
	if (!untitled)
...
void NPDGMain::PlotMonitorData(ERPlotTypes ptype, Int_t runNumber) {
	TCanvas *mc = NULL;
...
void NPDGMain::PlotMainData(ERPlotTypes ptype, Int_t runNumber) {
	TCanvas *mc = NULL;
	gStyle->SetPalette(1);
	...
	mc->cd(3);
	//draw RING1 channel-0

	cout << "Drawing RING1 channel-0 (Ring:0 det:0)" << endl;
	...
	mc->cd(4);
	//draw RING1 channel-12
	cout << "Drawing RING1 channel-12 (Ring:1 det:12)" << endl;
	r.Draw("d21[][12]:Iteration$","Entry$%25==0");
	...
	//draw RING1 channel-24
	cout << "Drawing RING1 channel-24 (Ring:2 det:24)" << endl;
	...
	mc->cd(6);
	//draw RING1 channel-36
	cout << "Drawing RING1 channel-36 (Ring:3 det:36)" << endl;
	r.Draw("d21[][36]:Iteration$","Entry$%25==0");
...
void NPDGMain::PlotDetectorData1(ERPlotTypes ptype, ENDataType dtype,
		Int_t runNumber) {
	TCanvas *mc = NULL;
	...
	mc = SplitCanvas(dDetCanvas1, 3, 4, "RING1");
	if (!mc)
		return;
	...
	for (Int_t i = 0; i < 12; i++) {
		...
		//plot ring 0 data
		    r.Draw(Form("d21[][%i]:Entry$", i), "Entry$%25==0");
...
void NPDGMain::PlotDetectorData2(ERPlotTypes ptype, ENDataType dtype,
		Int_t runNumber) {
	TCanvas *mc = NULL;
	...
	mc = SplitCanvas(dDetCanvas2, 3, 4, "RING2");
	if (!mc)
		return;

	//We have 12 channels per ring:  
...
void NPDGMain::PlotDetectorData3(ERPlotTypes ptype, ENDataType dtype,
		Int_t runNumber) {
	TCanvas *mc = NULL;
	...
	mc = SplitCanvas(dDetCanvas3, 3, 4, "RING3");
	if (!mc)
		return;

	//We have 36 channels per DAQ:  
	for (Int_t i = 24; i < 36; i++) {
		mc->cd(i - 23);
		//if(ptype ==  PT_HISTO_1D){
		if (i % 6 == 0)
			cout << endl;
...
void NPDGMain::PlotDetectorData4(ERPlotTypes ptype, ENDataType dtype,
		Int_t runNumber) {
	TCanvas *mc = NULL;
	...
	mc = SplitCanvas(dDetCanvas4, 3, 4, "RING4");
	if (!mc)
		return;
...
void NPDGMain::PlotSpinFlipperData(ERPlotTypes ptype, ENDataType dtype,
		Int_t runNumber) {
	TCanvas *mc = NULL;
...
Int_t NPDGMain::GetFilenameFromDialog(char *file, const char *ext,
...
Int_t NPDGMain::Append(char *buffer, Bool_t T_Stamp) {
...
Int_t NPDGMain::OpenLogFile(ERFileStatus status, const char* file) {
	if (IsLogFileOpen())
		CloseLogFile();
...
Bool_t NPDGMain::TabActive(const char *str) {
...
Bool_t NPDGMain::TabOpen(const char *str) {
...
Int_t NPDGMain::GetTabIndex(const char *str) {
	int nt = dTab->GetNumberOfTabs();
	for (int i = 0; i < nt; i++) {
		TString s = dTab->GetTabTab(i)->GetString();
		if (s == str) {
...
Int_t NPDGMain::WriteLogData(const char *filename) {
	if (!IsLogFileOpen()) {
...
void NPDGMain::OpenBrowser() {
	return;
}

void NPDGMain::CloseWindow() {
	// Got close message for this MainFrame. Terminate the application
	// or returns from the TApplication event loop (depending on the
	// argument specified in TApplication::Run()).

	sprintf(dMiscbuffer, "rm %s", "NPDGD_PID.DAT");
	system (dMiscbuffer);
	//CloseRun();
...
Bool_t NPDGMain::ProcessMessage(Long_t msg, Long_t parm1, Long_t parm2) {
...
int main(int argc, char **argv) {
	...
	if (help) {
		...
		NPDGMain mainWindow(gClient->GetRoot(), dClArgs, 1000, 800);
...
void NPDGMain::WritePid() {
	FILE *fp = fopen("NPDGD_PID.DAT", "w");
	if (fp) {
